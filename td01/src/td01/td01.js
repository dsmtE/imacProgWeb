...
export const allTheAnimals = [...animals1, ...animals2] // TODO
...
const makeDogACat = (thing) => { return (thing === 'dog') ? 'cat' : thing }
...
const number = 41
export const transformNumber = (number) =>  number + 1

export const makeSomethingOutOfNumber = (funct, number) => funct(number)
...
export const generateArticleWithSignature = (body, sig, name) => `${body} \n ${sig(name)}`
export const finalArticle = generateArticleWithSignature(articleBody, sig2, 'enguerrand DE SMET')
...
export const genAnimal = (name, species, legs, age, furColor) => ({
  name : name, species : species, legs : legs, age : age, furColor : furColor
})

// Create your animal of choice
export const sampleAnimal = genAnimal('blibli', 'squid', 18, 2, 'blue')
 ...
 */
export const mutatedHens = (hens) => {return hens.filter(hen => hen.legs != 2).map(hen => hen.name) }
...
export const averageAgeForHens = (hens) => hens.reduce( (sum, val) => sum + val.age , 0) / hens.length
...
export const specificHen = (hens) => hens.find(hen => hen.age > 15)
...
export const mergeObjects = (objects) => objects.reduce(( finalObject, obj) => Object.assign(finalObject, obj), {});

// Write the function that will take two arrays, merge them but remove duplicates.

export const union = (arr1, arr2) => [...arr1, ...arr2.filter(x => !arr1.includes(x))];
...
export const flatten = (arr) => 
  arr.reduce(function (flat, array) {
    return flat.concat(Array.isArray(array) ? flatten(array) : array);
  }, []);
