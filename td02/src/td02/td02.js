...
// export const every = (predicate, a) => a.reduce((acc, val) => acc && predicate(val), true)
export const every = (predicate, a) => a.filter(val => !predicate(val)).length <= 0
/**
 * Exercise 2
 *
...
export const div = (attributes, children) => createTag('div', attributes || null, children || [])
/**
 * Exercise 2.2
...
export const generateMarkupForAllDogs = (dogs) => createTag('div', null, dogs.map(dog => generateMarkupForDog(dog.url, dog.nom, dog.description)))
...
export const errorMessage = createTag(createTag('div', null, ['error message']))
